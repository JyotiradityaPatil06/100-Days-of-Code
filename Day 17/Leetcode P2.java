...
class Solution {
    public int[][] insert(int[][] intervals, int[] newInterval) {
        ...
        while (idx < intervals.length) {
            // If the current interval starts before the new interval, add it to the answer
            // list
            if (intervals[idx][0] < newInterval[0]) {
                ans.add(intervals[idx]);
                idx++;
                // If the current interval starts after or at the same time as the new interval,
                // exit the loop
            } else {
                break;
            }
        }
        // If the answer list is empty or the last interval in the answer list ends
        // before the new interval starts
        // add the new interval to the answer list
        if (ans.size() == 0 || (newInterval[0] > ans.get(ans.size() - 1)[1])) {
            ans.add(newInterval);
            // If the last interval in the answer list overlaps with the new interval, merge
            // them by updating the end time of the last interval
        } else {
        ...
        while (idx < intervals.length) {
            int[] lastInterval = ans.get(ans.size() - 1);
            // If the last interval in the answer list overlaps with the current interval,
            ...
            if (lastInterval[1] >= intervals[idx][0]) {
                ...
                // If the last interval does not overlap with the current interval, add it to
                // the answer list
                ans.add(intervals[idx]);
            }
            idx++;
